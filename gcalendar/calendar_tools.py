...
async def create_event(
    ...
    Args:
        ...
        color_id (Optional[str]): Color ID for the event (1-11). See Google Calendar API documentation for color meanings.
    ...
    # Set explicit visibility and transparency for calendar display
    event_body["visibility"] = "default"  # Ensure event is visible
    event_body["transparency"] = "opaque"  # Show as busy time
...
async def modify_event(
    ...
    # Set explicit visibility and transparency if not preserving existing
    if "visibility" not in event_body:
        event_body["visibility"] = "default"
    if "transparency" not in event_body:
